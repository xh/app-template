...
export class AuthModel extends HoistAuthModel {
    ...
    override async completeAuthAsync(): Promise<boolean> {
        ...
        if (!config.useOAuth) {
            ...
            const authResp = await this.useFormLogin();
            return authResp;
        }
        ...
        const authResp = await this.useAuth0Login(config);
        return authResp;
    }
    ...
    /**
     * Set application up to use form login
     */
    private async useFormLogin(): Promise<boolean> {
    ...
    /**
     * Set application up to use auth0 for login and session management
     */
    private async useAuth0Login(config: PlainObject): Promise<boolean> {
        const {audience, ...clientConfig} = config;
        this.client = new AuthZeroClient({
            ...
            audience,
            ...(clientConfig as AuthZeroClientConfig)
        });
